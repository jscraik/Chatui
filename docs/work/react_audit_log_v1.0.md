...
- [A11Y][DX] `packages/ui/src/components/ui/navigation/mode-selector.tsx`: added focus trap with Escape + focus restore, dialog semantics, non-interactive overlay, and explicit button type/ref handling.
- [CLEAN] `packages/ui/src/storybook/pages/DashboardPage.tsx`: removed redundant `cursor-pointer` class from `ListItem` (already applied when clickable).
...
- [A11Y][BUG] `packages/ui/src/components/ui/base/list-item.tsx`: avoid invalid `disabled` prop on non-button elements by only applying disabled/type to button variant.
- [BUG][DX] `packages/ui/src/storybook/pages/DashboardPage.stories.tsx`: fixed `ListItem` usage to use `label`/`selected` props instead of unsupported children/active prop.
- [SEC][A11Y] `platforms/web/apps/web/src/features/widgets/WidgetHarness.tsx`: added `noopener,noreferrer` when opening widget preview in a new tab.
- [A11Y][BUG] `packages/ui/src/components/ui/overlays/modal.tsx`: ensured `aria-describedby` always targets a real element and added hidden description node; stabilized IDs to avoid `undefined-description`.
- [A11Y][BUG] `packages/ui/src/components/ui/navigation/model-selector.tsx`: added `type="button"` to non-submit buttons and preserved disabled behavior for safer form embedding.
...
- [CLEAN][PERF] Replaced index keys with stable keys in lists:
  ...
  - `packages/ui/src/components/ui/navigation/mode-selector.tsx`
- [A11Y] `packages/ui/src/components/ui/base/toggle.tsx`: added accessible labeling props and wired `ToggleRow` to use `aria-labelledby`/`aria-describedby`.
...
- [A11Y] `packages/ui/src/components/ui/base/toggle.stories.tsx`: added `ariaLabel` for standalone Toggle story examples.
- [A11Y] `packages/ui/src/components/ui/base/segmented-control.tsx`: added `role="radiogroup"` and `role="radio"` + `aria-checked` for better screen reader support.
...
- [A11Y][BUG] `platforms/web/apps/web/src/features/widgets/WidgetHarness.tsx`: added explicit `type="button"` for widget selection buttons to prevent unintended form submits.
- [A11Y] `packages/ui/src/components/ui/overlays/tooltip.tsx`: added light fill token for tooltip arrow to avoid dark-only token usage.
...
- [A11Y] `packages/ui/src/components/ui/forms/range-slider.tsx`: added `useId` + `htmlFor` label wiring and fallback `aria-label` prop for unlabeled sliders.
- [A11Y] `packages/ui/src/components/ui/base/icon-button.tsx`: accepted standard `aria-label`/`aria-labelledby` props and wired them to the underlying button for lint/a11y parity.
- [A11Y] `packages/ui/src/components/ui/base/radio-group.tsx`: paired dark-only background token with a light-mode counterpart.
- [A11Y] `packages/ui/src/components/ui/base/select.tsx`: added light hover token to pair with dark hover styles under media-based theming.
- [A11Y] `packages/ui/src/components/ui/overlays/modal.tsx`: wired focus-trap ref to the dialog element to ensure correct focus management.
