...
export class ChatUIWidgetServer {
    ...
    async init(): Promise<void> {
        ...
        console.log(`ChatUI Widget Server initialized with ${Object.keys(widgetManifest).length} widgets`);
    }
                ...
                if (body.method === 'tools/list') {
                    return new Response(JSON.stringify({
                        tools: Array.from(this.tools.keys()).map(name => ({
                            name,
                            description: this.tools.get(name)?.description || `Tool: ${name}`,
                            inputSchema: this.tools.get(name)?.inputSchema || { type: "object", properties: {} }
                        }))
                    }), {
                        headers: {
                            'Content-Type': 'application/json',
    ...
    private async handleResourceRead(uri: string): Promise<Response> {
        try {
            ...
            const widgetEntry = Object.entries(widgetManifest).find(([, info]) =>
                `ui://widget/${(info as WidgetInfo).uri}` === uri
            );
            ...
            const resourceMeta = {
                ...
                "openai/widgetDomain": this.env.WIDGET_DOMAIN || this.env.WORKER_DOMAIN_BASE || "https://localhost:8787",
                "openai/widgetCSP": {
                    "connect_domains": [
            ...
            return new Response(JSON.stringify({ error: 'Failed to read resource' }), {
                status: 500,
                headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
            });
        }
    ...
    private registerExampleTools(): void {
        ...
        if (widgetManifest["dashboard-widget"]) {
            this.tools.set("show_dashboard", {
                description: "Display an interactive dashboard with analytics and quick actions for monitoring widget performance and system metrics",
                inputSchema: {
                    type: "object",
                    properties: {
                        timeRange: {
                ...
                _meta: {
                    ...
                    "openai/widgetDescription": "Interactive analytics dashboard showing widget performance, deployment status, and system metrics with real-time updates"
                },
                handler: async (args: Record<string, unknown>) => {
                    ...
                    return {
                        ...
                        structuredContent: {
                            ...
                            timeRange,
                            stats: [
                                { label: "Active Widgets", value: Object.keys(widgetManifest).length.toString(), change: "+2", trend: "up" },
        ...
        if (widgetManifest["enhanced-example-widget"]) {
            this.tools.set("show_enhanced_example", {
                description: "Display an enhanced example widget demonstrating OpenAI integration features including state management, theme detection, and tool calling",
                inputSchema: {
                    type: "object",
                    properties: {
                        demoMode: {
                ...
                _meta: {
                    ...
                    "openai/widgetDescription": "Interactive demonstration of advanced widget capabilities including persistent state, theme adaptation, and OpenAI API integration"
                },
                handler: async (args: Record<string, unknown>) => {
        ...
        if (widgetManifest["shopping-cart"]) {
            this.tools.set("show_shopping_cart", {
                description: "Display an interactive shopping cart with e-commerce functionality including add/remove items, quantity updates, and checkout flow",
                inputSchema: {
                    type: "object",
                    properties: {
                        items: {
                ...
                _meta: {
                    ...
                    "openai/widgetAccessible": true,
                    "openai/widgetDescription": "Interactive shopping cart with real-time updates, quantity controls, and checkout integration"
                },
                handler: async (args: Record<string, unknown>) => {
                    const items = args?.items as Array<{ id: string; name: string; price: number; quantity: number; image?: string }> || [];
                    const currency = args?.currency as string || "USD";
                    const total = items.reduce((sum, item) => sum + (item.price * item.quantity), 0);

                    return {
                        content: [{ type: "text", text: `Shopping cart with ${items.length} items (${currency} ${total.toFixed(2)})` }],
                        structuredContent: {
                            type: "shopping_cart",
                            currency,
                            items,
            ...
            this.tools.set("update_cart_item", {
                ...
                handler: async (args: Record<string, unknown>) => {
                    const { itemId, action, quantity } = args as { itemId: string; action: string; quantity?: number };
        ...
        if (widgetManifest["pizzaz-table"]) {
            this.tools.set("show_data_table", {
                description: "Display an interactive data table with sorting, filtering, and pagination capabilities for structured data visualization",
                inputSchema: {
                    type: "object",
                    properties: {
                        data: {
                ...
                _meta: {
                    ...
                    "openai/widgetAccessible": false,
                    "openai/widgetDescription": "Interactive data table with sorting, filtering, and pagination for structured data analysis"
                },
                handler: async (args: Record<string, unknown>) => {
                    ...
                    const customColumns = args?.columns as Array<{ key: string; label: string; sortable?: boolean; filterable?: boolean }>;

                    // Default sample data if none provided
                    const defaultData = [
                        { id: 1, name: "Widget Analytics", category: "Dashboard", status: "Active", usage: 1250, lastUpdated: "2025-12-28" },
                        { id: 2, name: "Shopping Cart", category: "E-commerce", status: "Active", usage: 890, lastUpdated: "2025-12-27" },
                        { id: 3, name: "Data Visualization", category: "Analytics", status: "Beta", usage: 456, lastUpdated: "2025-12-26" },
                        { id: 4, name: "File Manager", category: "Utility", status: "Active", usage: 234, lastUpdated: "2025-12-25" },
                        { id: 5, name: "Calendar Widget", category: "Productivity", status: "Pending", usage: 123, lastUpdated: "2025-12-24" }
                    ];
        ...
        if (widgetManifest["enhanced-example-widget"]) {
            this.tools.set("process_image", {
                description: "Process and analyze uploaded images with interactive preview and editing capabilities",
                inputSchema: {
                    type: "object",
                    properties: {
                        imageToProcess: {
                        ...
                        processingOptions: {
                            ...
                            properties: {
                                ...
                                filter: { type: "string", enum: ["none", "blur", "sharpen", "vintage"], default: "none" },
                ...
                _meta: {
                    ...
                    "openai/widgetDescription": "Interactive image processor with real-time preview and editing controls"
                },
                handler: async (args: Record<string, unknown>) => {
