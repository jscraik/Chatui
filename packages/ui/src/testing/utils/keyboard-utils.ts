 ...
 */
export async function getFocusableElements(page: Page, container: string): Promise<string[]> {
 ...
 */
export async function testShiftTabCycle(page: Page, _container: string): Promise<void> {
...
export async function testFocusTrap(
  ...
  for (let i = 0; i < focusableCount + 2; i++) {
    ...
    const isInContainer = await page
      .locator(`${container} :scope`)
      .evaluate((el, focusedEl) => el.contains(focusedEl), await focused.elementHandle());

    if (!isInContainer) {
      throw new Error(`Focus trap violation: Focus escaped container at Tab ${i + 1}`);
    }
  }
}
...
export async function testFocusRestoration(
  ...
  if (closeMethod === "escape") {
    ...
    await page.click(
      `${modalSelector} [aria-label="Close dialog"], ${modalSelector} button[aria-label="Close"]`,
    );
  }
  ...
  if (focusedId !== triggerId) {
    throw new Error(`Focus restoration failed: expected focus on #${triggerId}, got #${focusedId}`);
  }
}
 ...
 */
export async function testHomeEndKeys(page: Page, listSelector: string): Promise<void> {
  const items = await page.locator(`${listSelector} [role="menuitem"]`).all();
...
export async function runAxeScan(
  ...
  if (results.violations.length > 0) {
    if (process.env.DEBUG) {
      console.error("Axe violations detail:", JSON.stringify(results.violations, null, 2));
    }
