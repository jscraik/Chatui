...
import { ChatSidebar, type ChatSidebarUser, type SidebarItem } from "./ChatSidebar";
import { ComposeView, type ComposeModeConfig } from "./ComposeView";
import type { ModelConfig } from "../ui/navigation/model-selector";
  ...
  const baseSlots: ChatShellSlots = {
    ...
    messages:
      viewModeState === "compose" ? (
        ...
        <ChatMessages emptyState={emptyState} messages={messages} onMessageAction={onMessageAction} />
      ),
    composer:
...
export function ChatVariantContextRail({
  ...
  const resolvedSlots: Partial<ChatShellSlots> = {
    contextPanel: contextPanel ? (
      <div className="hidden lg:block w-[320px] border-l border-foundation-bg-light-3 dark:border-foundation-bg-dark-3 bg-foundation-bg-light-1 dark:bg-foundation-bg-dark-1">
        {contextPanel}
      </div>
    ) : null,
    ...slots,
