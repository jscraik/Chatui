 ...
 */
export function WidgetBase({ children, className = "", title }: WidgetProps) {
  return (
    <div className={`antialiased w-full text-white p-4 border border-white/10 rounded-2xl overflow-hidden bg-[var(--foundation-bg-dark-1)] widget-scrollable ${className}`}>
      {title && (
...
export function createWidget<T extends Record<string, any>>(
  ...
  return function Widget(props: T) {
    const component = (
      <WidgetBase 
        title={options?.title} 
        className={`${options?.className || ''} ${options?.maxHeight ? `max-h-[${options.maxHeight}]` : ''}`}
      >
        <Component {...props} />
      </WidgetBase>
    );

    // Auto-mount if this is the main widget file
